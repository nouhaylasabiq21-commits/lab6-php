...
class EtudiantService {
    ...
    public function create(EtudiantCreateDTO $dto) {
        ...
        $entity = new Etudiant(
                null, $dto->getCne(), $dto->getNom(), $dto->getPrenom(), $dto->getEmail(), $dto->getFiliereId()
        );
        return $this->etudiantDao->insert($entity);
    }

    public function update(EtudiantUpdateDTO $dto) {
        if ($dto->getId() <= 0) {
            throw new BusinessException('id étudiant invalide');
        }
        $this->validateEmail($dto->getEmail());
        ...
        if ($found === null) {
            throw new BusinessException('Etudiant introuvable');
        }
// Recréer pour simplicité (ou mettre à jour les champs sur l’entité existante)
        $entity = new Etudiant(
                $dto->getId(), $found->getCne(), // cne inchangé
                $dto->getNom(), $dto->getPrenom(), $dto->getEmail(), $dto->getFiliereId()
        );
    ...
    \App\Dto\FiliereCreateDTO $fDto, \App\Dto\EtudiantCreateDTO $eDto
    ) {
...
// forcer etudiant à pointer la filière créée
            $eDtoFixed = new \App\Dto\EtudiantCreateDTO(
                    $eDto->getCne(), $eDto->getNom(), $eDto->getPrenom(), $eDto->getEmail(), $fid
            );
...
// Propager en BusinessException si besoin
    ...
    private function validateCne(string $cne) {
