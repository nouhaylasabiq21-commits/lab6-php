            ...
            switch ($request['action']) {
                ...
                case 'create_etudiant':
                    $dto = new EtudiantCreateDTO(
                            (string) $request['cne'], (string) $request['nom'], (string) $request['prenom'], (string) $request['email'], (int) $request['filiere_id']
                    );
                    $id = $this->etudiantService->create($dto);
                    return new Response(true, ['id' => $id]);
                case 'update_etudiant':
                    $dto = new EtudiantUpdateDTO(
                            (int) $request['id'], (string) $request['nom'], (string) $request['prenom'], (string) $request['email'], (int) $request['filiere_id']
                    );
